...
static void hci_vendor_specific_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	case SEC_BRCM_DBFW_PLUS_DUMP_EVENT: {
		...
		if (event_code == DBFW_PLUS_A2DP_INFO_EVENT_CODE) {
			struct hci_cp_dbfw_plus_a2dp_debug cp;

			cp.subcommand = DBFW_PLUS_A2DP_INFO_EVENT_CODE;
			cp.enable = 0x00;
			memcpy(&cp.handle, ev->data, 2);
			BT_INFO("[%s(%d)] Disable DBFW+ A2DP Debug Info(handle: 0x%4.4x)", __FUNCTION__, __LINE__, __le16_to_cpu(cp.handle));
			hci_send_cmd(hdev, HCI_OP_DBFW_PLUS_LINKLOSS, sizeof(cp), &cp);
		}
#endif /* kernel never control 'DBFW+ A2DP Debug Info' feature */
