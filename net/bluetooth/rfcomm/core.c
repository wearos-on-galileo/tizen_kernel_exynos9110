...
#ifdef CONFIG_TIZEN_WIP
	BT_INFO("[%s(%d)] session: %p, state: %s(%ld), channel: %d, MTU: %d", __FUNCTION__, __LINE__, s, state_to_string(s->state), s->state, __srv_channel(d->dlci), cpu_to_le16(pn->mtu));
#endif
	return rfcomm_send_frame(s, buf, ptr - buf);
}
...
#ifdef CONFIG_TIZEN_WIP
			BT_INFO("[%s(%d)] session: %p, state: %s(%ld), channel: %d, MTU: %d", __FUNCTION__, __LINE__, s, state_to_string(s->state), s->state, __srv_channel(d->dlci), d->mtu);
#endif
