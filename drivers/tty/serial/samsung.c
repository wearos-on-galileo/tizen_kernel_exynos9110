...
#ifdef BT_UART_TRACE
	unsigned int ch_str[BT_TX_RX_BUFFER_SIZE] = {0, };
	int ch_str_cnt = 0;
#endif
	spin_lock_irqsave(&port->lock, flags);
...
#ifndef PATCH_FIRST_DIV_NEXT_AFC
	/* Move from here */
	umcon = rd_regl(port, S3C2410_UMCON);
	if (termios->c_cflag & CRTSCTS) {
				   ...
				   struct s3c2410_uartcfg *cfg)
{
	...
	/*
	if ((ourport->pm_dbg_mode & UART_DBG_MODE) && (ourport->port.line == BLUETOOTH_UART_PORT_LINE)) {
		dev_notice(port->dev, "[%s(%d)] To prevent IRQ storm, Need to mask interrupt before enabling the channel!\n", __FUNCTION__, __LINE__);
	}
	*/
