...
extern int wristup_work_initialized;
extern struct wristup_cpu_pmqos_work_s wristup_cpu_booster_work;
...
static BLOCKING_NOTIFIER_HEAD(touch_event_notifier_list);


int touch_event_register_notifier(struct notifier_block *nb, unsigned int list)
{
	int ret;

	ret = blocking_notifier_chain_register(&touch_event_notifier_list, nb);
...
int touch_event_unregister_notifier(struct notifier_block *nb, unsigned int list)
{
	int ret;

	ret = blocking_notifier_chain_unregister(&touch_event_notifier_list, nb);
...
#if defined(CONFIG_SLP_BUSY_LEVEL)
	time_begin = jiffies;
	if (load_analyzer_init_done) {
		blocking_notifier_call_chain(&touch_event_notifier_list,
			TOUCH_EVENT_PRESS, &time_begin);
	}
#endif
...
int lcd_power_wristup_tracer = 0;
int lcd_aod_is_enabled = 0;
/* PmQoS request for enhance wristup wake up performance */
void wristup_booster_set_lcd_power(int lcd_power)
{
	lcd_power_wristup_tracer = lcd_power;
}

void wristup_booster_set_aod_state(int aod_state)
{
	lcd_aod_is_enabled = aod_state;
}

void wristup_booster_turn_on(void)
{
	/* PmQoS request for enhance wristup wake up performance */
	if ((!work_busy(&wristup_cpu_booster_work.wristup_cpu_pmqos_work)) &&
		(wristup_work_initialized) && ((lcd_power_wristup_tracer == 4) ||
		(lcd_aod_is_enabled &&  !lcd_power_wristup_tracer))) {
		queue_work_on(0, wristup_cpu_booster_work.wristup_cpu_pmqos_workqueue,
				&wristup_cpu_booster_work.wristup_cpu_pmqos_work);
	}
}
