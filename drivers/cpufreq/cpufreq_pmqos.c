...
#define PERIOD_IDLE_CHECK (60 * 60 * 1000) /*1 hour*/
#define PERIOD_IDLE_CHECK_1 (PERIOD_IDLE_CHECK + 150 * 1000) /*1 hour and 2.5 minutes*/
#define PERIOD_IDLE_CHECK_2 (PERIOD_IDLE_CHECK + 300 * 1000) /*1 hour and 5 minutes*/
#define PERIOD_IDLE_CHECK_3 (PERIOD_IDLE_CHECK + 450 * 1000) /*1 hour and 7.5 minutes*/
extern unsigned int get_cpu_idle_state(void);
static unsigned int is_dump = 0;
#endif
...
#if defined (CONFIG_SLP_BUSY_LEVEL)
#define SAVE_BUSY_LEVEL_PERIOD		(120 * 1000) /* 2 minutes */
static unsigned long last_busy_level_updated = 0;
...
void update_cpu_busy_level(void)
{
	...
	if (jiffies_to_msecs(cur_time - last_busy_level_updated) >= SAVE_BUSY_LEVEL_PERIOD) {
		sysfs_notify(cpufreq_pmqos_kobject, NULL, "cpu_busy_level_value");
		last_busy_level_updated = cur_time;
...
void check_idle_cpu_state(void)
{
	...
	if (begin == 0) begin = jiffies;
	if (jiffies_to_msecs(jiffies - begin) >= PERIOD_IDLE_CHECK_3) {
		is_dump = 1;

		if (!cpufreq_pmqos_kobject)
			return;

		sysfs_notify(cpufreq_pmqos_kobject, NULL, "is_dump_log");
		pr_info("%s check 3\n", __FUNCTION__);
		begin = jiffies;
		return;
	} else if (jiffies_to_msecs(jiffies - begin) >= PERIOD_IDLE_CHECK_2) {
		if (get_cpu_idle_state()) {
			is_dump = 1;
	...
	} else if (jiffies_to_msecs(jiffies - begin) >= PERIOD_IDLE_CHECK_1) {
		if (get_cpu_idle_state()) {
			is_dump = 1;
	...
	} else if (jiffies_to_msecs(jiffies - begin) >= PERIOD_IDLE_CHECK) {
		if (get_cpu_idle_state()) {
			is_dump = 1;
...
static ssize_t show_is_dump_log(struct kobject *kobj,
		struct attribute *attr, char *buf)
{
	unsigned int ret = 0;
...
define_one_root_ro(is_dump_log);
#endif
