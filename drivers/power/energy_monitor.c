...
/* penalty score value */
#define ENERGY_MON_SCORE_VERSION "1.1"
#define ENERGY_MON_SCORE_SBM_FAST_DRAIN	BIT(17)
	...
	int skip_penalty_check)
{
	...
	badness = alarm_wakeup - (int)aod_offset;
	if (badness > 0 && badness > energy_mon.penalty.threshold_alarm)
		score = ENERGY_MON_SCORE_ALARM;

	energy_mon_dbg(ENERGY_MON_DEBUG_INFO,
		"%s: %d: score=0x%x %d %d %d\n", __func__, p_curr->log_count, score,
		badness, energy_mon.penalty.threshold_alarm, aod_offset);
...
#ifdef CONFIG_INPUT_STAT
	input_stat_get_stat_delta(type, &p_curr->input_stat);
#endif
#ifdef CONFIG_SENSORHUB_STAT
...
#ifdef CONFIG_SID_SYS_STATS
#ifdef CONFIG_PID_SYS_STATS
	get_sid_cputime2(type, p_curr->sid_cpu, p_curr->pid_cpu, SID_CPUTIME_ARRAY_SIZE);
#else
	get_sid_cputime(type, p_curr->sid_cpu, SID_CPUTIME_ARRAY_SIZE);
#endif
...
#endif
#ifdef CONFIG_SLEEP_STAT
	sleep_stat_get_stat_delta(type, &p_curr->sleep_stat);
#endif
#ifdef CONFIG_SENSORHUB_STAT
...
#ifdef CONFIG_INPUT_STAT
static ssize_t energy_monitor_print_input_stat(char *buf,
				int buf_size, enum energy_mon_print_type p_type)
{
	ssize_t ret = 0, ret_temp = 0;
	struct energy_mon_data *p_curr = NULL;
	...
	char *buf)
{
	...
	ret += scnprintf(buf, PAGE_SIZE,
			...
			diff_wakeup[4],
			diff_wakeup[5],
			(int)wakeup_time_pct[0],
			(int)wakeup_time_pct[1],
			(int)wakeup_time_pct[2],
			(int)wakeup_time_pct[3],
			(int)wakeup_time_pct[4],
			(int)wakeup_time_pct[5],
			(int)wakeup_time_pct[6],
			(int)wakeup_time_pct[7],
			p_curr->input_stat.ev_wakeup[EV_WU_00],
			p_curr->input_stat.ev_wakeup[EV_WU_10],
			p_curr->input_stat.ev_wakeup[EV_WU_20],
			p_curr->input_stat.ev_wakeup[EV_WU_30],
			p_curr->input_stat.ev_wakeup[EV_WU_40],
			p_curr->input_stat.ev_wakeup[EV_WU_GT_40],
			...
			btdn,
			bttm,
			p_curr->slwl[0].slwl_name,
			(int)ktime_to_timeval(p_curr->slwl[0].prevent_time).tv_sec,
			p_curr->slwl[1].slwl_name,
			(int)ktime_to_timeval(p_curr->slwl[1].prevent_time).tv_sec,
			p_curr->slwl[2].slwl_name,
			(int)ktime_to_timeval(p_curr->slwl[2].prevent_time).tv_sec,
			p_curr->ws[0].name,
			(int)ktime_to_timeval(p_curr->ws[0].emon_total_time).tv_sec,
			p_curr->ws[1].name,
			(int)ktime_to_timeval(p_curr->ws[1].emon_total_time).tv_sec,
			p_curr->ws[2].name,
			(int)ktime_to_timeval(p_curr->ws[2].emon_total_time).tv_sec,
			&usid_buf[0][0],
			...
			gpu_freq[5],
			gpu_freq[6],
			(int)ktime_to_ms(p_curr->ff_stat.total_time),
			(int)p_curr->ff_stat.play_count,
			p_curr->sleep_stat.fail,
			p_curr->sleep_stat.failed_freeze,
			p_curr->sleep_stat.failed_prepare,
			p_curr->sleep_stat.failed_suspend,
...
static int __init energy_monitor_dt_init(void)
{
	...
	energy_mon_dbg(ENERGY_MON_DEBUG_INFO,
		"%s: %lld %d %s %s %x %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
		__func__,
		energy_mon.unit_bat_capacity,
		energy_mon.use_raw_soc,
