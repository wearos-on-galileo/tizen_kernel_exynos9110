...
static int ub_gpio_irq;
static irqreturn_t dsim_ub_con_det_irq_handler(int irq, void *dev_id)
{
	struct dsim_device *dsim = dev_id;

	if (!dsim->ub_con_enable) {
		decon_info("%s : ub_con is not enabled [%d]\n", __func__, dsim->ub_con_intr_cnt);
		return IRQ_HANDLED;
	}

	dsim->ub_con_intr_cnt++;
	decon_info("%s : occur interrupt[%d][%d]\n", __func__,
		dsim->ub_con_intr_cnt, gpio_get_value(ub_gpio_irq));
...
#ifdef CONFIG_SEC_SYSFS
		ub_gpio_irq = of_get_gpio(dev->of_node, 1);
		dsim->ub_con_intr = gpio_to_irq(ub_gpio_irq);
		dsim->ub_con_intr_cnt = 0;
		ret = devm_request_irq(dev, dsim->ub_con_intr, dsim_ub_con_det_irq_handler,
			IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "ub_det", dsim);
		if (ret < 0) {
			dsim_err("failed to request det irq.\n");
		} else {
			dsim_info("dsim_ub_con_det_irq_handler register[%d]\n", dsim->ub_con_intr_cnt);
			disable_irq(dsim->ub_con_intr);
			dsim->ub_con_enable = 0;
		}
#endif
...
static ssize_t dsim_store_ub_con_det(struct device *dev, struct device_attribute *devattr, const char *buf, size_t size)
{
        ...
        if (cmd == 0) {
                if (dsim->ub_con_enable) {
                        dsim_info("disable ub_con_det test\n");
                        disable_irq(dsim->ub_con_intr);
                }
                dsim->ub_con_enable = 0;
        }
                ...
                if (!dsim->ub_con_enable) {
						dsim_info("enable ub_con_det test\n");
                        enable_irq(dsim->ub_con_intr);
						cancel_delayed_work(&dsim->ub_con_dwork);
						schedule_delayed_work(&dsim->ub_con_dwork, msecs_to_jiffies(2000));
                }
				else
					dsim->ub_con_enable = 1;
        }
        else if (cmd == 2) {
                dsim_info("clear ub_con_det count\n");
                dsim->ub_con_intr_cnt = 0;
        }
...
static void dsim_ub_con_dwork(struct work_struct *work)
{
	struct dsim_device *dsim = container_of(work,
				struct dsim_device, ub_con_dwork.work);

	dsim_info("set ub_con_det test\n");
	dsim->ub_con_enable = 1;

}

static int dsim_init_ub_con_check(struct dsim_device *dsim)
{
        struct device *sec_dev;
        int ret = 0;

        sec_dev = sec_device_create(dsim, "lcd");
        if (unlikely(!sec_dev)) {
                dsim_err("%s: ailed to create factory dev\n", __func__);
                return -ENODEV;
        }

        ret = sysfs_create_group(&sec_dev->kobj, &ub_con_attr_group);
        if (unlikely(ret)) {
                dsim_err("%s: Failed to create ub_con sysfs group\n", __func__);
                return ret;
        }

		INIT_DELAYED_WORK(&dsim->ub_con_dwork, dsim_ub_con_dwork);
...
static int dsim_parse_dt(struct dsim_device *dsim, struct device *dev)
{
	...
	dsim->dev = dev;
	of_property_read_string(dev->of_node, "model_name", &dsim->lcd_info.model_name);
