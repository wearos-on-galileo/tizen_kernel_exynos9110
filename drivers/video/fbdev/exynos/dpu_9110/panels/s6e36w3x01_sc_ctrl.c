...
void s6e36w3x01_write_sideram_afpc(struct s6e36w3x01 *lcd)
{
	struct dsim_device *dsim = lcd->dsim;
	int i;
	const char area_array[6][5] = {{0x2b, 0x00, 0x00, 0x01, 0x67},
	...
	if (dsim_wr_data(dsim->id, MIPI_DSI_DCS_LONG_WRITE,
		(unsigned long)area_array[6],
		ARRAY_SIZE(area_array[6])) < 0)
		dsim_err("failed to send area_array[6].\n");
