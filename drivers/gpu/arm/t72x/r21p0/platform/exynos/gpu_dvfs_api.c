...
#if defined (CONFIG_SYSTEM_LOAD_ANALYZER)
	store_external_load_factor(GPU_FREQ, target_clk);
#endif
...
int gpu_dvfs_init_time_in_state(void)
{
	...
	for (i = gpu_dvfs_get_level(platform->gpu_max_clock); i <= gpu_dvfs_get_level(platform->gpu_min_clock); i++) {
		...
		platform->table[i].emon_time = 0;
		platform->table[i].delta_time = 0;
#endif
	}
...
#if defined(CONFIG_GPUFREQ_STAT)
		platform->table[level].emon_time += current_time-prev_time;
		platform->table[level].delta_time += current_time-prev_time;
#endif
	}
...
#if defined(CONFIG_CHECK_TIME_IN_STATE)
	if (jiffies_64_to_clock_t(current_time -time_state_a )  * 1000 / CLOCKS_PER_SEC >= TIME_IN_STATE_INTERVAL) {
		for (i = gpu_dvfs_get_level(platform->gpu_min_clock); i >= gpu_dvfs_get_level(platform->gpu_max_clock); i--) {
			la_get_time_in_state(i,platform->table[i].clock,(unsigned long long)
				jiffies_64_to_clock_t(platform->table[i].time) * 1000 / CLOCKS_PER_SEC, "LA_GPU");
		}
		time_state_a = current_time;
	}
#endif
#endif /* CONFIG_MALI_DEBUG_SYS */
...
#if defined(CONFIG_SYSTEM_LOAD_ANALYZER)
void la_gpufreq_stats_update(int clock)
{
	gpu_dvfs_update_time_in_state(clock);
}
EXPORT_SYMBOL(la_gpufreq_stats_update);
#endif
