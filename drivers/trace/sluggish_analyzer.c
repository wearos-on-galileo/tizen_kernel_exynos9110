...
static int check_mem_warning(void)
{
	...
	if (DIFF_VMSTAT(ALLOCSTALL) > 20) /* experimental value */
		return 1;
#else
	if (DIFF_VMSTAT(ALLOCSTALL_NORMAL) > 20) /* experimental value */
		return 1;
...
static void check_warning(void)
{
	...
	if (period_in_msec && time_after(now, timeout)) {
		...
		if (check_io_wait_warning()) {
			warning++;
#if defined(CONFIG_SLP_BUSY_LEVEL)
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_IOWAIT] = busy_state_get_time_occur();
			pr_info("%s: [BUSY_LEVEL] Trigger reason: Iowait long time\n", __FUNCTION__);
#endif
		...
		if (check_mem_warning()) {
			warning++;
#if defined(CONFIG_SLP_BUSY_LEVEL)
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_MEM_ALLOCSTALL] = busy_state_get_time_occur();
			pr_info("%s: [BUSY_LEVEL] Trigger reason: Memory AllocStall\n", __FUNCTION__);
#endif
			#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4,7,0))
