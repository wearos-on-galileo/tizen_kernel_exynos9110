...
static void exynos_ss_set_core_power_stat(unsigned int val, unsigned cpu)
{
...
#if 0 //defined(CONFIG_SEC_SIPC_MODEM_IF)
...
int exynos_ss_post_panic(void)
{
	if (ess_base.enabled) {
		...
		if ((__raw_readl(exynos_ss_get_base_vaddr() +
				ESS_OFFSET_SCRATCH) == ESS_SIGN_SCRATCH) && sec_debug_enter_upload())
#else
		if (__raw_readl(exynos_ss_get_base_vaddr() +
