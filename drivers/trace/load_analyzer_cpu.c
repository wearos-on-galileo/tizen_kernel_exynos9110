...
int get_last_framerates(void)
{
	...
	pr_info("%s: frame rate: %d", __FUNCTION__, total_frames * 100 / total_vsync);

	return total_frames * 100 / total_vsync;
}
#endif
...
{
	pr_info("%s: trigger calculate framerate\n", __FUNCTION__);

#if defined(CONFIG_FRAME_DROP_DETECTOR)
	if (get_last_framerates() < FRAME_DROP_THRESHOLD) {
		cpu_busy_level = TARGET_BUSY_STATE;
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_FRAME_DROP] = busy_state_get_time_occur();

		pr_info("%s: [BUSY_LEVEL] Trigger reason: Frame drop\n", __FUNCTION__);
         ...
         unsigned long event, void *data)
{
	...
	if (event == TOUCH_EVENT_RELEASE) {
		time_end = data;
		if ((time_end != NULL) && (time_begin != NULL)) {
...
static int last_cpu_load_read_sub(char *buf, int buf_size)
{
	...
	if (busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_IOWAIT]) {
		ret +=  snprintf(buf + ret, buf_size - ret, "Iowait - Timestamp (s): %lu.%lu, ",
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_IOWAIT] / 1000,
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_IOWAIT] % 1000);
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_CPU_IOWAIT] = 0;
	}
	if (busy_level_time_trigger[BUSY_LEVEL_TRIGGER_MEM_ALLOCSTALL]) {
		ret +=  snprintf(buf + ret, buf_size - ret, "MemAllocstall - Timestamp (s): %lu.%lu, ",
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_MEM_ALLOCSTALL] / 1000,
			busy_level_time_trigger[BUSY_LEVEL_TRIGGER_MEM_ALLOCSTALL] % 1000);
		busy_level_time_trigger[BUSY_LEVEL_TRIGGER_MEM_ALLOCSTALL] = 0;
	}
#if defined(CONFIG_FRAME_DROP_DETECTOR)
	...
	do {
		...
		frames_per_sec = total_frames * 1000 / FRAMERATE_DETECT_PERIOD;
		ret +=  snprintf(buf + ret, buf_size - ret, "%d fps (%d/%2d)\n\n", frames_per_sec, total_frames,total_vsync);
