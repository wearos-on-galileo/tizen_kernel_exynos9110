...
unsigned long g_rx_buffer_avail_bytes = HSI_PZC_MAX_RX_BUFFER; // Initial value doesn't matter
...
int bcm_ssi_rx(struct bcm_spi_priv *priv, size_t *length)
{
	...
	if (payload_len == 0) {
		pr_err("[SSPBBD]: %s @ RX length is 0\n", __func__);
		return -1;
	}
	...
	do {
		...
		if ( avail ) {
			...
			if ( avail > g_rx_buffer_avail_bytes ) {
				...
				pr_info("[SSPBBD] %s - %d PZC %s, wr CIRC_CNT %lu, RNGDMA_RX %lu \n",
						__func__, g_rx_buffer_avail_bytes ? ssi_tx_pzc_retries:ssi_tx_pzc_retry_delays,
						g_rx_buffer_avail_bytes ? "writes":"delays", avail, g_rx_buffer_avail_bytes);
...
static irqreturn_t bcm_irq_handler(int irq, void *pdata)
{
	...
	/* we don't want to queue work in suspending and shutdown */
	if (!atomic_read(&priv->suspending)){
		wake_lock_timeout(&priv->bbd_wake_lock, HZ/4);
...
*/
	...
	wake_lock_init(&priv->bbd_wake_lock, WAKE_LOCK_SUSPEND, "bbd_wake_lock");
	g_bcm_gps = priv;
	/* Init BBD & SSP */
...
{
	if (!g_bcm_gps) {
		printk("[SSPBBD]: WARN!! bcm_gps didn't yet init, or removed\n");
		return;
	}

	stat1hz.host_req = gpio_get_value(g_bcm_gps->host_req);
	stat1hz.mcu_req = gpio_get_value(g_bcm_gps->mcu_req);
	stat1hz.mcu_resp = gpio_get_value(g_bcm_gps->mcu_resp);

	stat1hz.irq_enabled = atomic_read(&g_bcm_gps->irq_enabled);
	stat1hz.irq_count = kstat_irqs_cpu(g_bcm_gps->spi->irq, 0);
}
#endif
