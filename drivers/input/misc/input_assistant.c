...
static void input_assistant_init_tsp(struct input_handle *handle)
{
	...
	if (id_info->maximum == 0)
		tsp_data->max_finger = TSP_NUM_MAX;
	else
		tsp_data->max_finger = id_info->maximum;

	pr_debug("%s:[%s][%d]", __func__, dev->name, tsp_data->max_finger);
}
			       ...
			       const struct input_value *vals, unsigned int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		case ABS_MT_TRACKING_ID:
			...
			tsp_data->in_data[tsp_data->last_slot].finger_id = tsp_data->last_slot;
			tsp_data->in_data[tsp_data->last_slot].update = true;
			update = true;
			break;
		case ABS_MT_SLOT:
		...
		case ABS_MT_PALM:
			...
			pr_info("%s:[%s][%d] Palm, e=%d\n", __func__,
				...
				tsp_data->in_data[tsp_data->last_slot].finger_id,
				tsp_data->event_cnt);
			break;
		}
	}
	...
	if (update) {
		...
		for (i = 0; i < tsp_data->max_finger; i++) {
			if (tsp_data->in_data[i].update) {
				tsp_data->in_data[i].update = false;
				pr_info("%s:[%s][%d] x=%d, y=%d, z=%d, M=%d, m=%d, e=%d\n", __func__,
					tsp_data->in_data[i].tracking_id == TSP_FINGER_RELEASE ? "R":"P",
					tsp_data->in_data[i].finger_id, tsp_data->in_data[i].x,
					tsp_data->in_data[i].y,	tsp_data->in_data[i].z,
					tsp_data->in_data[i].wmajor, tsp_data->in_data[i].wminor,
					tsp_data->event_cnt);
			}
		}
	}
