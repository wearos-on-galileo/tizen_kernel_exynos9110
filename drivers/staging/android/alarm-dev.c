...
static void alarm_clear(enum android_alarm_type alarm_type)
{
	...
	if (!alarm_pending && !wait_pending) {
		...
		pr_info("%s: __pm_relax fail alarm_pending: 0x%x wait_pending: 0x%x\n",
			__func__, alarm_pending, wait_pending);
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ANDROID_ALARM_SET_TZ:
		...
		return 0;
	case ANDROID_ALARM_CREATE_PROCESS:
		if (copy_from_user(&pid, (void __user *)arg, sizeof(pid)))
			return -EFAULT;
		rv = add_alarm_history(ALARM_HISTORY_TYPE_CREATE, pid);
		return rv;
	case ANDROID_ALARM_EXPIRE_PROCESS:
		if (copy_from_user(&pid, (void __user *)arg, sizeof(pid)))
			return -EFAULT;
		rv = add_alarm_history(ALARM_HISTORY_TYPE_EXPIRE, pid);
		return rv;
	}
							...
							unsigned long arg)
{
	...
	case ANDROID_ALARM_GET_TIME_COMPAT(0):
		cmd = (unsigned int)ANDROID_ALARM_COMPAT_TO_NORM(cmd);
		break;
	}
...
static void devalarm_triggered(struct devalarm *alarm)
{
	...
	if (alarm_enabled & alarm_type_mask) {
		...
		pr_info("%s: __pm_wakeup_event fail alarm_enabled: 0x%x alarm_type_mask: 0x%x\n",
			__func__, alarm_enabled, alarm_type_mask);
	spin_unlock_irqrestore(&alarm_slock, flags);
}
