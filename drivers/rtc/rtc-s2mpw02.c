...
#ifdef CONFIG_SEC_PM
static bool is_rtc_cleared = false;
#endif
				 ...
				 enum S2M_RTC_OP op)
{
	...
	ret = s2mpw02_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, data);
	if (ret < 0)
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = s2mpw02_bulk_read(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mpw02_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_check_rtc_time(struct s2m_rtc_info *info)
{
	...
	if (abs(rtc_time - sys_time.tv_sec) > 2) {
		...
		ret = s2mpw02_bulk_write(info->i2c, S2MP_RTC_REG_SEC,
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpw02_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mpw02_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mpw02_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
				data);
		if (ret < 0) {
						...
						struct s2mpw02_platform_data *pdata)
{
	...
	if (pdata->osc_bias_up >= 0) {
		ret = s2mpw02_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->osc_xin >= 0) {
		ret = s2mpw02_update_reg(info->i2c, S2MP_RTC_REG_OSCCTRL,
...
static bool s2m_is_jigonb_low(struct s2m_rtc_info *info)
{
	...
	ret = s2mpw02_read_reg(info->i2c, reg, &val);
	if (ret < 0) {
						...
						struct s2mpw02_platform_data *pdata)
{
	...
	ret = s2mpw02_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mpw02_platform_data *pdata)
{
	...
	ret = s2mpw02_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mpw02_platform_data *pdata)
{
	...
	ret = s2mpw02_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &update_val);
	if (ret < 0) {
	...
	ret = s2mpw02_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, info->update_reg);
	if (ret < 0) {
	...
	ret = s2mpw02_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mpw02_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mpw02_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	capsel_val |= 0xf0;
	ret = s2mpw02_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, capsel_val);
	if (ret < 0) {
	...
	if (pdata->init_time) {
		...
		time_history_rtc_time_set(pdata->init_time, ret);
	}
	else {
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	struct s2mpw02_platform_data *pdata = dev_get_platdata(iodev->dev);
	struct s2m_rtc_info *info;
	int irq_base;
	int ret = 0;

	pr_info("S2MPW02 RTC probe start\n");
	...
	switch (info->iodev->device_type) {
	case S2MPW02X:
		info->irq = irq_base + S2MPW02_PMIC_IRQ_RTCA0_INT2;
		break;
	default:
		/* If this happens the core funtion has a problem */
		BUG();
