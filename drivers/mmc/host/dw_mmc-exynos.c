...
void dw_mci_reg_dump(struct dw_mci *host)
{
	...
	dev_err(host->dev, ": ================ GPIO REG ===============\n");
	dev_err(host->dev, ": GPIO_GPF0_CON:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF0_CON));
	dev_err(host->dev, ": GPIO_GPF0_DAT:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF0_DAT));
	dev_err(host->dev, ": GPIO_GPF0_PUD:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF0_PUD));
	dev_err(host->dev, ": GPIO_GPF0_DRV:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF0_DRV));
	dev_err(host->dev, ": GPIO_GPF1_CON:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF1_CON));
	dev_err(host->dev, ": GPIO_GPF1_DAT:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF1_DAT));
	dev_err(host->dev, ": GPIO_GPF1_PUD:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF1_PUD));
	dev_err(host->dev, ": GPIO_GPF1_DRV:	 0x%08x\n",
			readl(host->mmc_gpio + MMC_GPF1_DRV));
	dev_err(host->dev, ": ===========================================\n");
...
#endif /* CONFIG_GALILEO || CONFIG_PULSE */
