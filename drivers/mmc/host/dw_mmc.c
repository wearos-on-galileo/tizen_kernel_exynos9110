...
static int dw_mci_command_complete(struct dw_mci *host, struct mmc_command *cmd)
{
	...
	if (status & SDMMC_INT_RTO)
		...
		if (!(cmd->opcode == MMC_SEND_TUNING_BLOCK ||
			...
			dev_err(host->dev, "cmd response crc error, status 0x%08x\n",
					status);
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	host->mmc_gpio = ioremap(MMC_GPIO, 0x8000);
	if (!host->mmc_gpio) {
		dev_err(host->dev, "GPIO debug memory alloc faile!\n");
		goto debug_alloc_fail;
	}
