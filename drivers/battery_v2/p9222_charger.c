...
static void p9222_wpc_send_tx_uno_mode_disable(struct p9222_charger_data *charger, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		p9222_reg_write(charger->client, P9222_RX_DATA_COMMAND, 0x19);
		p9222_reg_write(charger->client, P9222_RX_DATA_VALUE0, 0xFF);
...
static void p9222_check_abnormal_charge_status(struct p9222_charger_data *charger)
{
	int i;
	...
	if (p9222_power_sharing_charge_chk(charger, charger->tx_id)) {
		for (i = 0; i < 3; i++) {
			...
			if (p9222_unsafe_charge_check(charger) == 0)
				return;
		}
		p9222_wpc_send_tx_uno_mode_disable(charger, 3);
	}
}
...
{
	int toggle_time = charger->pdata->ping_duration * 2;
	int i;
			...
			if (gpio_is_valid(charger->pdata->wpc_en)) {
				for (i = 0; i < 2; i++) {
					...
					if (p9222_check_chg_in_status(charger) == 1) {
						...
						charger->err_status &= ~P9222_ERR_PHM_FREE;
						break;
					} else {
						pr_info("%s phm free fail!(%d)\n", __func__, i);
						charger->err_status |= P9222_ERR_PHM_FREE;
					}
				}
			} else {
...
static int p9222_monitor_work(struct p9222_charger_data *charger)
{
	...
	if (p9222_is_on_pad(charger) == WPC_ON_PAD) {
		...
		pr_info("%s vrect(%dmV) vout(%dmV) iout(%dmA) vbat(%dmV) ap_mode(0x%02x) SOC(%d) "
			"phm(%d) chg_type(%d) chg_mode(%d) "
			"fw(%x) vbat_monitor(%x) tx_id(%x) freq(%d) temp(%d) full(%x) err(%x)\n",
		...
		if (p9222_unsafe_charge_check(charger)) {
			p9222_check_abnormal_charge_status(charger);
		}
	} else {
		pr_info("%s pad off!\n", __func__);
	}
...
#define OP_FREQ_OFFSET_PLUS_0KZ			0x00
#define OP_FREQ_OFFSET_PLUS_5KZ			0x01
#define OP_FREQ_OFFSET_PLUS_10KZ		0x02
#define OP_FREQ_OFFSET_MINIUS_5KZ		0xFE
#define OP_FREQ_OFFSET_MINIUS_10KZ		0xFD

static void p9222_set_op_freq_offset(struct p9222_charger_data *charger, int data)
{
	switch (data) {
		case OP_FREQ_OFFSET_PLUS_0KZ:
			charger->op_freq_offset = 0;
			break;
		case OP_FREQ_OFFSET_PLUS_5KZ:
			charger->op_freq_offset = 5;
			break;
		case OP_FREQ_OFFSET_PLUS_10KZ:
			charger->op_freq_offset = 10;
			break;
		case OP_FREQ_OFFSET_MINIUS_5KZ:
			charger->op_freq_offset = -5;
			break;
		case OP_FREQ_OFFSET_MINIUS_10KZ:
			charger->op_freq_offset = -10;
			break;
		default:
			charger->op_freq_offset = 0;
	...
	pr_info("%s offset data = 0x%x offset = %d\n",
		__func__, data, charger->op_freq_offset );
}
...
static void p9222_wpc_send_rx_id(struct p9222_charger_data *charger, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		p9222_reg_write(charger->client, P9222_PACKET_HEADER, P9222_HEADER_AFC_CONF); // using 2 bytes packet property
		p9222_reg_write(charger->client, P9222_RX_DATA_COMMAND, P9222_RX_DATA_COM_RX_ID);
		p9222_reg_write(charger->client, P9222_RX_DATA_VALUE0, charger->pdata->rx_id);
		p9222_reg_write(charger->client, P9222_COMMAND_REG, P9222_CMD_SEND_RX_DATA);
...
static irqreturn_t p9222_wpc_irq_thread(int irq, void *irq_data)
{
	...
	if(irq_src[0] & P9222_STAT_TX_DATA_RECEIVED_MASK) {
		...
		if (data1 == P9222_TX_DATA_COM_FREQ_OFFSET) {
			p9222_set_op_freq_offset(charger, data2);
		} else {
	...
	if (p) {
		...
		if (ret) {
			...
			for (i = 0; i < pdata->num_compatible_tx; i++)
				pr_info("charger_type tx_id = 0x%02x power_hold = %d tx_pad_type(%d) cc(%x) cv(%x) phm_toggle(%d)\n",
