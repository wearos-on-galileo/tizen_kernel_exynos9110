		...
		const char __user *buf, size_t count, loff_t *pos)
{
	...
	char tmp_buff[100] = {0};
	if (count >= 100){
		pr_err("%s: user buff size invalid", __func__);
		return -EINVAL;
	}
	if (copy_from_user(tmp_buff, buf, count))
		return -EFAULT;

	if (sscanf(tmp_buff, "%31s %63s", option, val) < 0)
		return -EINVAL;
	if (strcasecmp(option, "timezone") == 0) {
		...
		if (sscanf(val, "%ld", &newtime.tv_sec) < 0)
			pr_err("%s: %s time get failed", __func__, option);
